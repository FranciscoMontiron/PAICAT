...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <h1 class="text-3xl font-bold text-gray-800">Editar Rol: {{ $role->nombre }}</h1>
            <p class="text-gray-600 mt-1">Modifica los datos y permisos del rol</p>
        </div>
    </div>
    ...
    <form action="{{ route('roles.update', $role) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="flex items-start gap-3 p-4 border rounded-xl cursor-pointer transition-all hover:border-utn-blue-dark/30 has-[:checked]:border-utn-blue-dark has-[:checked]:bg-utn-blue/5">
                        <input type="checkbox" name="solo_contenido_asignado" value="1"
                            {{ old('solo_contenido_asignado', $role->solo_contenido_asignado) ? 'checked' : '' }}
                    ...
                    <label class="flex items-start gap-3 p-4 border rounded-xl cursor-pointer transition-all hover:border-utn-blue-dark/30 has-[:checked]:border-utn-blue-dark has-[:checked]:bg-utn-blue/5">
                        <input type="checkbox" name="visibilidad_general" value="1"
                            {{ old('visibilidad_general', $role->visibilidad_general) ? 'checked' : '' }}
                            class="mt-0.5 h-4 w-4 text-utn-blue-dark border-gray-300 rounded focus:ring-utn-blue-dark">
                        <div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-2.5 bg-utn-blue text-white rounded-lg hover:bg-utn-dark transition-colors">Guardar Cambios</button>
        </div>
    </form>
